...
These data are from Fearn, T. (1983). A misuse of ridge regression in the calibration of a near infrared reflectance instrument.

The data are the result of an experiment to calibrate a near infrared reflectance (NIR) instrument for measuring the protein content of ground wheat samples.  The protein content of each sample (y in percent) was measured by the standard Kjeldahl method. The predictors, L1-L6, are measurements of th ereflectance of NIR radiation by the wheat samples at 6 wavelength in the range 1680-2310 nm.  The measurements were made on the log(1/reflectance) scale.  The problem is to find a linear combination of the predictors that predicts protein content. The estimated coefficients can then be entered into the instrument allowing the protein content of future samples to be read directly.  In Fearn (1983) the first 24 cases were used for calibration and the last 26 samples were used for prediction.  ~%"))

(let* ((names '("Protein" "L1" "L2" "L3" "L4" "L5" "L6"))
